...
async def create_ad_account_with_docids(
    ...
    *,
    ...
    if not capture_doc_id.isdigit() or not captured_variables:
        raise AdAccountMutationError(
            "CREATE_AD_ACCOUNT_LIVE_CAPTURE_REQUIRED",
            (
                "ReMask requires an exact live-captured Add-RK GraphQL request "
                "from this profile before CREATE. No CREATE was sent."
            ),
            retryable=True,
        )

    captured_variables_rewritten = _replace_capture_values(
        captured_variables,
    ...
    if not captured_variables_rewritten:
        raise AdAccountMutationError(
            ...
            (
                "The live Add-RK capture did not contain usable variables. "
                "No CREATE was sent."
            ),
            retryable=True,
        )
    ...
    try:
        response = await browser_graphql(
            ...
            request_envelope=(
                ...
                if isinstance(capture.get("request_envelope"), dict)
                else {}
            ),
        ...
        if failure_kind == "stale_schema":
            raise AdAccountMutationError(
                ...
                (
                    "Meta rejected the mutation/schema captured from the live "
                    "wizard. Re-capture is required; no fallback doc_id will be "
                    "tried. " + diagnostic
                ),
                retryable=True,
                payload=payload,
    ...
    if account_id:
        ...
        persisted = stored if isinstance(stored, DocIdCandidate) else candidate
        record_result(
            CREATE_AD_ACCOUNT_OPERATION,
            persisted,
            success=True,
    ...
    if isinstance(data, dict) and data:
        raise AdAccountMutationError(
            "CREATE_AD_ACCOUNT_RESULT_UNKNOWN",
            (
                "Meta returned data for the live-captured CREATE_AD_ACCOUNT but "
                "ReMask could not prove an Ad Account ID. CREATE will not be "
                "repeated until inventory reconciliation succeeds."
            ),
            retryable=True,
            payload=response,
            candidate=candidate,
