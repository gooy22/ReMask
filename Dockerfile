...
RUN /opt/remask-venv/bin/python -m compileall -q /opt/remask-python \
    ...
    && /opt/remask-venv/bin/python -m unittest -q tests.test_fb_worker_bootstrap tests.test_business_private_first \
    ...
    && echo "[bm-private-first-v11] DTSG + Page fallback + private-first BM tests passed"
