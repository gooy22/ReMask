...
RUN set -eux; \
    ...
    php -r '$pairs=[["/var/www/html/accounts.php","",18000],["/var/www/html/scripts/index.js","",14000],["/var/www/html/scripts/selection-persistence.js","",12000],["/var/www/html/ajax/storageStatus.php","",10000]]; foreach($pairs as [$file,$needle,$len]){$s=file_get_contents($file);fwrite(STDERR,"[accounts-inspect] FILE=".$file."\\n".substr($s,0,$len)."\\n");}'; \
