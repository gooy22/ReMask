...
# Temporary one-shot sync verification. It logs only hashed profile ids and counts.
if [ -f "$ROOT/bin/remask-sync-smoke.php" ]; then
  ( sleep 3; php "$ROOT/bin/remask-sync-smoke.php" 2>&1 || true ) &
fi
